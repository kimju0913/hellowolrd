...
<body>
   ...
   <div id="mainpageMain">
      ...
      <div id="seachMainWrapper">
         <div id="searchInputWrapper">
            <input type="text" id="searchInput" placeholder="이름 이나 일촌명을 입력해주세요." onkeyup="selectFriends(this)" >
            <button id="btnSearch">
               <img id="imgMagnifier"
         ...
         <c:forEach var="vo" items="${frList }" varStatus="status">
           <div id = "${status.index }" style = "display : block; ">
            <div id="FriendWrapper" >
               <label class="lblFriendName" id = "test${status.index }">${vo.fr_user_name }</label>
               <div id="pInfoWrapper">
                  <p>(${vo.nickname })</p>
               </div>
               <div id="FriendBtnWrapper">
                  <input type = "button" id="btnUpdateFrd"
                     onclick="goToChangeNickFrd('${vo.fr_id}','${vo.fr_user_name }')" value = "일촌명  변경">
                  
                  <input type = "button" id="btntest"
                     onclick="goHompie('${vo.fr_id}')" value = "미니홈피">
                     
                  <input type = "button" id="btnDeleteFrd"
 ...
 // 검색 기능 ~~~~~~~~~~~~~~~~~      
    ...
    function selectFriends(target){
      var inputSearch = $(target).val();
      var size = ${frList.size()};
      for(var x = 0 ; x < size; x ++){
      $("#"+ x).css("display","none");
      if( $("#"+ x +" :contains("+inputSearch+")")){
      $("#"+x +":contains("+inputSearch+")").css("display","block");
      } 
     }
